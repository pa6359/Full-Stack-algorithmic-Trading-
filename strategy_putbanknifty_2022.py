...
def calculate_EMA_with_signals(df):
    ...
    future_data.to_csv("mydf_putBN_2022.csv")
    ...
    for index, row in future_data.iterrows():
        if row['signal'] == 'Buy':
            ...
            if all([buy_atmsp, buy_date, buy_time, expiry_date]):
                # Fetch option data
                OptionCE = query(f_o='O', instrument='BANKNIFTY', expiry_dt=expiry_date, strike=row['ATMSP'], option_type="PE", start_date=pd.to_datetime(pd.to_datetime(row['Date']).date()), end_date=pd.to_datetime(pd.to_datetime(row['Date']).date()))
...
tradedf.to_csv("mytradedf_putBN_2022.csv")
